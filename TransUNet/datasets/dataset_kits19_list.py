...
class KiTS19DatasetList(Dataset):
    ...
    def _normalize(self, volume):
        ...
        mask = volume != 0
        if mask.sum() > 0:  # Check if we have any non-zero values
            mean = volume[mask].mean()
            std = volume[mask].std()
            if std == 0:  # Handle constant non-zero values
                return np.zeros_like(volume)  # or return volume - mean
            normalized = (volume - mean) / (std + 1e-8)
            return normalized
        return volume  # Return original if all zeros
    ...
    def __getitem__(self, idx):
        ...
        # Return additional information for testing
        return {
            "image": image, 
            "label": label,
            "case_id": case,
            "slice_idx": slice_idx,
            "file_name": f"{case} {slice_idx}"  # Format matching your list file
        }